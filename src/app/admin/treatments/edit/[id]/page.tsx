...
export default function TreatmentEditPage() {
  ...
  const { result, query } = useShow<TreatmentData>({
    ...
    meta: { select: 'id,name,species_id,category,description,active,is_periodic,reminder_interval_value,reminder_interval_unit' },
  });
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(error.message || 'خطا در به‌روزرسانی نوع درمان. لطفاً دوباره امتحان کنید.');
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-6">
      <div>
        <div className="flex items-center gap-3"><h1 className="font-display text-2xl font-bold text-foreground">ویرایش نوع درمان</h1><PageHelp id="treatments-edit" /></div>
      ...
      {submitSuccess && (
        <div className="rounded-app border border-primary bg-primary/10 p-4 text-center text-primary">
          نوع درمان با موفقیت به‌روزرسانی شد!
        </div>
      )}
