...
export default function ServiceShowPage() {
  const { result, query } = useShow<ServiceData>({ resource: 'services', meta: { select: 'id,name,description,price_rial,category,duration_minutes,is_active,doctor_id,doctor:doctors(*)' } });
  ...
  {result.doctor?.role && ` - ${result.doctor.role}`}
</p></div>
