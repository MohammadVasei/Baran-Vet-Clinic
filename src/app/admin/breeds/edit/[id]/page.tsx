...
export default function BreedEditPage() {
  const { result, query } = useShow<BreedData>({
    resource: 'breeds',
    meta: { select: 'id,name,species_id,active' },
  });
  const { mutateAsync: updateBreed, mutation } = useUpdate();
  ...
  if (!result) return <div className="p-8 text-center text-destructive">نژاد یافت نشد.</div>;
    ...
    <form onSubmit={submit} className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="space-y-5 rounded-app-lg border border-border bg-surface p-6">
        ...
        <div>
          ...
          <Input id="breed-name" value={name} onChange={(event) => setName(event.target.value)} required className="mt-2" />
        </div>
