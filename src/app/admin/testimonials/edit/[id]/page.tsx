...
export default function TestimonialEditPage() {
  const { result, query } = useShow<TestimonialData>({
    resource: 'testimonials',
    meta: { select: 'id,name,quote,rating,animal_type,pet_note,display_order,is_published' },
  });
  const { mutateAsync: updateTestimonial, mutation } = useUpdate();
  ...
  useEffect(() => {
    ...
    setHasAnimal(!!result.animal_type);
    setAnimalType(result.animal_type || 'dog');
    setPetNote(result.pet_note || '');
  ...
  if (!result) return <div className="p-8 text-center text-destructive">بازخورد یافت نشد.</div>;
