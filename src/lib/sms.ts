...
export async function sendShippingSMS(data: {
  ...
  if (data.status === 'shipped') {
    const trackingInfo = data.trackingNumber
      ? `شماره پیگیری: ${data.trackingNumber}`
      : 'بدون شماره پیگیری';
    const courierInfo = data.courier ? `- ${data.courier}` : '';

    message = `سفارش شما ارسال شد. ${trackingInfo}${courierInfo}. برای تحویل با ما تماس بگیرید.`;
  } else {
    // data.status must be 'delivered' here since it's a union type
    message = `سفارش شما تحویل گرفته شد. متشکر از خرید شما.`;
  }

  if (!apiKey || isDev) {
    console.log('📱 [DEV MODE] Shipping SMS would be sent to:', data.phone);
    console.log('📱 [DEV MODE] Message:', message);
    return;
  }
