...
export function FacilitiesMobile() {
  ...
  return (
    <section id="facilities" className="relative overflow-hidden bg-background py-20">
      <div className="container-site relative">
        <MobileSectionHeader
          eyebrow={FACILITIES.eyebrow}
          headline={FACILITIES.headline}
          intro={FACILITIES.intro}
        />

        <div className="mt-8 -mx-6" ref={containerRef}>
          <SnapCarousel
            ariaLabel="فضاهای کلینیک"
            slideClassName="w-[82%]"
            showDots={false}
            onIndexChange={setActiveIndex}
          >
            {FACILITIES.items.map((item, i) => (
              <FacilityCard key={item.key} item={item} index={i} />
            ))}
          </SnapCarousel>
          ...
          <ProgressIndicator
            ...
            active={activeIndex}
            accent={FACILITIES.items[activeIndex].key as FacilityAccentKey}
          />
        </div>
      </div>
    </section>
...
function FacilityCard({ item, index }: { item: Facility; index: number }) {
  const accent = FACILITY_ACCENTS[item.key as FacilityAccentKey] ?? FACILITY_ACCENTS.surgery;
...
function ProgressIndicator({
  ...
  count: number;
  active: number;
  accent: FacilityAccentKey;
}) {
  const accentClasses = FACILITY_ACCENTS[accent] ?? FACILITY_ACCENTS.surgery;
